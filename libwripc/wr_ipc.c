...
wripc_handle_t wripc_create_server(const char *name)
{
  ...
  if((rval = bind (fd, (struct sockaddr *)&sun, sizeof(struct sockaddr_un))) < 0)
    {
      perror("bind");
      close(fd);
      return rval;
  ...
  srv->funcs = safe_zmalloc(WRIPC_MAX_FUNCS * sizeof(struct wripc_function));
  srv->events = safe_zmalloc(WRIPC_MAX_FUNCS * sizeof(struct wripc_event));
  srv->conns = safe_zmalloc(WRIPC_MAX_FUNCS * sizeof(struct wripc_connection));
...
//  unlink(sun.sun_path);
  ...
  if((rval = connect (fd, (struct sockaddr *)&sun, sizeof(struct sockaddr_un))) < 0)
    {
...
static struct wripc_function *find_function(struct wripc_server_context *srv, const char *func_name)
{
	int i;
	
	for(i=0;i<WRIPC_MAX_FUNCS;i++)
	 if(srv->funcs[i].in_use && !strcmp(func_name, srv->funcs[i].name))
	   return &srv->funcs[i];
	   
	 return NULL;
}

static struct wripc_function *new_function(struct wripc_server_context *srv, const char *func_name)
{
	int i;
	
	if(find_function(srv, func_name) != NULL) // function already registered
		return NULL;
...
static inline struct wripc_function *lookup_function(struct wripc_server_context *srv, const char *func_name)
{
	int i;
	for(i=0;i < WRIPC_MAX_FUNCS; i++)
		if(srv->funcs[i].in_use)
...
static inline int deserialize_string(uint32_t *stream, int current_pos, int buf_size, uint32_t *dst)
{
...
static inline int deserialize_struct(uint32_t *stream, int current_pos, int buf_size, uint32_t *dst)
{
...
static inline int serialize_string(uint32_t *buffer, int current_pos, int max_pos, char *str)
{
	int len;
	int num_words;
...
static inline int serialize_struct(uint32_t *buffer, int current_pos, int max_pos, int size, void *ptr)
{
...
static inline int send_call_reply(struct wripc_connection *conn, struct wripc_function *func, uint32_t *rval, int buf_size)
{
  int total_size;
	...
	if(func->rval_type == T_VOID)
	{
		...
		switch(func->rval_type)
		{
			...
			case T_DOUBLE:
				...
				break;
			case T_STRING:
				total_size = serialize_string(buf, 2, MAX_MESSAGE_SIZE, (char *)rval);
				if(total_size < 0) {
...
//				printf("ReturnsStruct: size %d\n", func->rval_size);
				total_size = serialize_struct(buf, 2, MAX_MESSAGE_SIZE, func->rval_size, rval);
...
static int handle_call(struct wripc_server_context *srv, struct wripc_connection *conn, uint32_t *buf, int buf_size)
{
	struct wripc_function *func;
	char *func_name;
...
if(func->rval_type == T_INT8 || func->rval_type == T_INT16 || func->rval_type == T_INT32 || func->rval_type == T_VOID)
  {
 		direct_rval = 1; // these functions can return the value directly (e.g. int add (a, b) { return a+b;  }
 		arg_buf_pos = 0;
  } else {
 		direct_rval = 0;
 		arg_buf_pos = 1;
	...
	for(i=0;i<num_args;i++)
	{
		...
		switch(arg_type)
		{
			case T_INT8: arg_buf[arg_buf_pos++] = buf[current_pos++] & 0xff; break;
			case T_INT16: arg_buf[arg_buf_pos++] = buf[current_pos++] & 0xffff; break;
			case T_INT32: arg_buf[arg_buf_pos++] = buf[current_pos++] & 0xffffffff; break;
			...
			case T_STRING: 
				current_pos = deserialize_string(buf, current_pos, buf_size, &arg_buf[arg_buf_pos++]);
			...
			case T_STRUCT_TYPE:
				current_pos = deserialize_struct(buf, current_pos, buf_size, &arg_buf[arg_buf_pos++]);
				DBG("StructArg, pos= %d\n",current_pos);
...
static int handle_client_request(struct wripc_server_context *srv, struct wripc_connection *conn)
{
	  ...
	  if(conn->in_use)
	  {
			...
			pfd.events = POLLIN | POLLHUP | POLLERR ;
	 		...
	 		if(poll(&pfd, 1, 0) > 0)
	 		{
	 			...
	 			{
	 				DBG("poll returned error, killing connection (fd = %d)\n", conn->cli_fd);
	 				conn->in_use = 0;
	 				close(conn->cli_fd);
	 			} else if(pfd.revents & POLLIN) {
	 				 			
	 				int rv = handle_client_request(srv, conn);
	 				
	 				DBG("handle request returned %d\n", rv);
	 				
	 				if(rv < 0) 
	 				{
	 					DBG("Request failed. Closing connection");
	 					conn->in_use = 0;
	 					close(conn->cli_fd);
	 				}
	 			}
...
//	DBG("dupa?");
  ...
  va_start(ap, num_args);
	...
	for(i = 0; i<num_args; i++)
	{
		...
		switch(type)
		{
			case T_INT8:  buffer[current_pos++] = va_arg(ap, uint32_t) & 0xff; break;
			case T_INT16: buffer[current_pos++] = va_arg(ap, uint32_t) & 0xffff; break;
			case T_INT32: buffer[current_pos++] = va_arg(ap, uint32_t) ; break;
			case T_FLOAT:
			case T_DOUBLE: {
...
//				DBG("serialize: T_STRING\n");
 				int new_pos = serialize_string(buffer, current_pos, MAX_MESSAGE_SIZE, va_arg(ap, char *));
			...
			case T_STRUCT_TYPE: {
			  ...
			  new_pos = serialize_struct(buffer, current_pos, MAX_MESSAGE_SIZE, struct_size, struct_ptr);
			  if(new_pos < 0) return WRIPC_ERROR_NO_MEMORY;
		   	current_pos = new_pos;
...
//  fprintf(stderr,"\n");
  ...
  DBG("total size: %d, sent: %d\n", current_pos*4, rv);	
	...
	rv = recv_with_timeout(cli->fd, buffer, 4*MAX_MESSAGE_SIZE, REPLY_TIMEOUT);

	if(rv <= 0 || rv != buffer[1]) return WRIPC_ERROR_MALFORMED_PACKET;

	if(buffer[0] == MSG_TYPE_ERROR) // oops. we received an error
		return (int) buffer[2];
	else if (buffer[0] == MSG_TYPE_CALL_ACK) // ack packet contains the return value
	{
...
static int do_subscribe(wripc_handle_t handle, int event_id, int onoff)
{
	...
	if(recv_with_timeout(cli->fd, buf, sizeof(buf), REPLY_TIMEOUT) != sizeof(buf))
		return WRIPC_ERROR_MALFORMED_PACKET;
	
	if(buf[0] != MSG_TYPE_SUBSCRIBE_ACK || buf[1] != sizeof(buf) || buf[2] != event_id)
		return WRIPC_ERROR_MALFORMED_PACKET;
