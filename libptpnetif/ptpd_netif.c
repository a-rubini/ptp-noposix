  ...
  if(tmo_expired(&s->dmtd_update_tmo))
    {
      ...
      s->dmtd_phase = pstate.phase_val; // FIXME: ccheck if phase value is ready

      tmo_restart(&s->dmtd_update_tmo);
    }
}

static void linearize_rx_timestamp(wr_timestamp_t *ts, wr_socket_t *sock, int cntr_ahead)
{
  struct my_socket *s = (struct my_socket *) sock;
  int trip_lo, trip_hi;
  ...
  if(inside_range(trip_lo, trip_hi, s->dmtd_phase))
    {
      ...
      // as the "reliable" one. cntr_ahead will be 1 when the rising edge counter is 1 tick ahead of the falling edge counter
      ...
      // to simulate a timestamp transition exactly at s->phase_transition DMTD phase value
...
int ptpd_netif_calibration_pattern_enable(const char *ifaceName, unsigned int calibrationPeriod, unsigned int calibrationPattern, unsigned int calibrationPatternLen)
{
  ...
  if( (ret = halexp_calibration_cmd(ifaceName,HEXP_CAL_CMD_CHECK_IDLE,HEXP_ON)) != HEXP_CAL_RESP_OK)
  {
    netif_dbg("(PTPD_NETIF): Calibrating other interface, returned %d, attempt to send calibratin patern on interface %s FAILED !!\n",ret, ifaceName);
    return PTPD_NETIF_NOT_READY;
  }
  
  if((ret = halexp_calibration_cmd(ifaceName,HEXP_CAL_CMD_TX_PATTERN,HEXP_ON)) != HEXP_CAL_RESP_OK)
  {
    netif_dbg("(PTPD_NETIF): returned %d, attempt to start sending calibration pattern on interface %s FAILED !!\n", ret,ifaceName);
      return PTPD_NETIF_NOT_READY;
  }
  netif_dbg("(PTPD_NETIF): Started sending calibration pattern on port %s SUCCESSFULLY !!\n",ifaceName);


#else
  
  netif_dbg("(PTPD_NETIF): start sending calibration pattern[%s] for %d us on interface = %s\n",ptpd_netif_netif_dbg_bits(calibrationPattern,calibrationPatternLen),calibrationPeriod,ifaceName);
...
int ptpd_netif_calibrating_disable(int txrx, const char *ifaceName)
{
  ...
  if(txrx == PTPD_NETIF_RX)
  {
     ...
     */
    if((ret = halexp_calibration_cmd(ifaceName,HEXP_CAL_CMD_RX_MEASURE,HEXP_OFF)) != HEXP_CAL_RESP_OK)
    {
      netif_dbg("(PTPD_NETIF): returned %d, attempt to stop RX calibration  interface %s FAILED !!\n", ret,ifaceName);
      return PTPD_NETIF_ERROR;
    }
    netif_dbg("(PTPD_NETIF): Stopped RX calibrating interface %s SUCCESSFULLY !!\n",ifaceName);
  }
  else  if(txrx == PTPD_NETIF_TX)
  {
     ...
     */    
    if((ret = halexp_calibration_cmd(ifaceName,HEXP_CAL_CMD_TX_MEASURE,HEXP_OFF)) != HEXP_CAL_RESP_OK)
    {
      netif_dbg("(PTPD_NETIF): returned %d, attempt to stop calibration of interface %s FAILED !!\n", ret,ifaceName);
      return PTPD_NETIF_NOT_READY;
    }
    netif_dbg("(PTPD_NETIF): Stopped TX calibrating interface %s SUCCESSFULLY !!\n",ifaceName);
  }
  else
    return PTPD_NETIF_ERROR;
...
#else
  if(txrx == PTPD_NETIF_TX)
    netif_dbg("(PTPD_NETIF): disable calibrating TX, interface: %s\n",ifaceName);
  else
    netif_dbg("(PTPD_NETIF): disable calibrating RX, interface: %s\n",ifaceName);    
#endif
...
int ptpd_netif_calibrating_enable(int txrx, const char *ifaceName)
{
  ...
  if(txrx == PTPD_NETIF_RX)
  {
     ...
     * stop calibration of reception delay (RX)
     */
    if((ret = halexp_calibration_cmd(ifaceName,HEXP_CAL_CMD_RX_MEASURE,HEXP_ON)) != HEXP_CAL_RESP_OK)
    {
      netif_dbg("(PTPD_NETIF): returned %d, attempt to sttart RX calibration  interface %s FAILED !!\n", ret,ifaceName);
      return PTPD_NETIF_ERROR;
    }
    netif_dbg("(PTPD_NETIF): Started RX calibrating interface %s SUCCESSFULLY !!\n",ifaceName);
  }
  else  if(txrx == PTPD_NETIF_TX)
  {
    /*
     * stop calibration of transmision delay (TX)
     */    
    if((ret = halexp_calibration_cmd(ifaceName,HEXP_CAL_CMD_TX_MEASURE,HEXP_ON)) != HEXP_CAL_RESP_OK)
    {
      netif_dbg("(PTPD_NETIF): returned %d, attempt to start calibration of interface %s FAILED !!\n", ret,ifaceName);
      return PTPD_NETIF_NOT_READY;
    }
    netif_dbg("(PTPD_NETIF): Started TX calibrating interface %s SUCCESSFULLY !!\n",ifaceName);
  }
  else
    return PTPD_NETIF_ERROR;
...
#else
  if(txrx == PTPD_NETIF_TX)
    netif_dbg("(PTPD_NETIF): enable calibrating TX, interface: %s\n",ifaceName);
  else
    netif_dbg("(PTPD_NETIF): enable calibrating RX, interface: %s\n",ifaceName);    
#endif
...
int ptpd_netif_calibration_pattern_disable(const char *ifaceName)
{
  ...
  if((ret = halexp_calibration_cmd(ifaceName,HEXP_CAL_CMD_TX_PATTERN,HEXP_OFF)) != HEXP_CAL_RESP_OK)
  {
    netif_dbg("(PTPD_NETIF): returned %d, attempt to stop sending calibration pattern on interface %s FAILED !!\n", ret,ifaceName);
      return PTPD_NETIF_ERROR;
  }
  netif_dbg("(PTPD_NETIF): Stopped sending calibration pattern on port %s SUCCESSFULLY !!\n",ifaceName);
#else
  
  netif_dbg("(PTPD_NETIF): stop sending calibration pattern,  interface = %s\n",ifaceName);
...
//	printf("(ptpd_netif) linkdown detected on port: %s\n",ifaceName);
  ...
  printf("(ptpd_netif) linkdown detected on port: %s [no valid port state data)\n",ifaceName);
  //should not get here
  return PTPD_NETIF_ERROR;
}
