...
int wr_servo_init(PtpClock *clock)
{
  ...
  s->delta_rx_s = ((int32_t)clock->deltaRx.scaledPicoseconds.lsb) >> 16;
	...
	ptpd_wrap_strncpy(cur_servo_state.sync_source, clock->netPath.ifaceName, 16);//fixme
	ptpd_wrap_strncpy(cur_servo_state.slave_servo_state, "Uninitialized", 32);
	servo_state_valid = 1;
...
printf("delta_RX_S = %d\n", s->delta_rx_s);
  ...
  delay_ms = ((double)ts_to_picos(s->mu) - big_delta) / (2.0 + alpha) // fiber part
    + (double)s->delta_tx_m + (double) s->delta_rx_s + ph_adjust; // PHY/routing part
