...
# else
#  define DBG(x, ...)  fprintf(stderr, "(DBG [p=%d] wrMode: %s%s%s) " x, ptpClock->portIdentity.portNumber,(ptpClock->isWRmode== TRUE ? "ON->" : "OFF "),(ptpClock->wrMode== WR_MASTER ? "MASTER" : ""),(ptpClock->wrMode== WR_SLAVE ? "SLAVE" : ""), ##__VA_ARGS__)
# endif
#else
# define DBG(x, ...)
#endif
